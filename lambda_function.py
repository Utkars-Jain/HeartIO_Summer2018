...
def lambda_handler(event, context):
    ...
    if event:
        ...
        fileObj = s3.get_object(Bucket = "xml-conversion", Key = filename)
    ...
    filepath = os.path.join(tmppath2, base_Name + ".csv")
    
    datafromfile = open(filepath)
    
    s3.put_object(Bucket = "xml-conversion", Body = datafromfile, Key = filepath[5:])
